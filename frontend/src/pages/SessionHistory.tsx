  ...
  return (
    <>
      ...
      <div className="filters">
        {filters.map((f) => (
          <button key={f} className={"filter-pill" + (filter === f ? " on" : "")} onClick={() => setFilter(f)}>{t("history." + f)}</button>
        ))}
      </div>
      <div className="panel reveal">
        <div className="tbl-scroll">
          <table className="tbl">
            <thead><tr><th>{t("history.thExercise")}</th><th>{t("history.thMode")}</th><th>{t("history.thDate")}</th><th>{t("history.thQuality")}</th><th>{t("history.thBand")}</th><th>{t("history.thScore")}</th><th></th></tr></thead>
            <tbody>
              {rows.map((r) => (
                <tr key={r.id}>
                  ...
                  <td style={{ textAlign: "right" }}><Link className="btn btn-ghost" to={`/report?session=${r.id}`} style={{ padding: "7px 14px" }}>{t("history.view")}</Link></td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
        {!loading && rows.length === 0 && <p className="muted center" style={{ padding: "28px 0" }}>{t("history.empty")}</p>}
      </div>
    </>
  );
}
