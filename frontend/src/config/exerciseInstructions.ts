// Per-exercise content for the shared <ExerciseInstructions> page.
// Add a new entry here to support another exercise without changing the page.
...
export interface ExerciseInstructionConfig {
  ...
  exerciseCode: string;
  titleKey: string;
  /** i18n key for the short non-diagnostic purpose and benefit blurb. */
  whyKey: string;
  demoSrc: string;
  /** i18n keys for numbered steps, including any plain-language limits or consequences. */
  stepKeys: string[];
  cameraAngle: {
    image: string;
    captionKey: string;
  ...
  equipmentKey?: string;
  /** SLS-only ball/line live-overlay explainer. */
  overlayExplainer?: {
    titleKey: string;
    bodyKey: string;
    image: string;
