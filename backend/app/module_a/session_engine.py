...
class SessionEngine:
    ...
    def _run_wblt(self, frames: list[dict]) -> dict:
        """Weight-Bearing Lunge Test: tracks trials, measures ankle dorsiflexion ROM and symmetry.

        Returns trial count, avg dorsiflexion angle, and symmetry proxy (angle difference L-R).
        """
        if not frames:
            return self._empty_wblt_result()
        ...
        metrics = {
            ...
            "max_dorsiflexion_deg": (
                round(max_ankle_angle_this_trial, 2)
                if trial_dorsiflexion_angles
                else 0.0
            ),
