...
| Report metric display: exact degrees vs bands only | The normalisation section (§12.3, and the linked proposal) stated the system would use **banding and relative features** specifically **to avoid the system obtaining an exact degree as clinical measurements** — no raw angle was to be shown to the user | **Option A retained: exact degree values are shown.** The Report page displays precise numeric angles (`knee_rom_deg`, `avg_trunk_lean_deg`, `leg_angle_deg`) alongside the Good/Fair/Poor band, not bands alone | Two things justify keeping this, together: (1) UAT found users repeatedly wanted more numeric detail, not less (e.g. "the post-session report should explain every metric," "squat: angle number ... user didn't saw" — several sessions in `docs/UAT_OBSERVATION_ANALYSIS.md`), so a bands-only report was measured to under-inform real users; (2) it remains defensible **only because of a specific, measured finding**: Stage 5.4's mocap-agreement check (`ml/reports/MOCAP_AGREEMENT.md`) compared this pipeline's `knee_flex_peak_deg` (from MediaPipe world landmarks) against OptiTrack 26-joint motion-capture ground truth on the same 98 REHAB24-6 reps, and found a systematic bias of **−11.96°** (pipeline reads lower than mocap; 95% limits of agreement [−21.00°, −2.94°]; ICC(2,1) = 0.726; Pearson r = 0.956, n = 98) — high correlation (the _shape_ of the movement is tracked faithfully) but poor absolute agreement (the _magnitude_ is systematically off). That report's own conclusion is explicit: this "does not license treating our degrees as clinical degrees" — any angle shown is "on its own scale, not a goniometer's." So the as-built compromise is to show the exact number **for transparency and trend-tracking**, never framed or labelled as a clinical measurement, with the band remaining the primary, safety-relevant output. The same −11.96° figure is reused elsewhere: it is exactly why the squat depth fault gate is set at 78.04° rather than the clinical 90° parallel norm (90° − 11.96° ≈ 78.04°, per `backend/app/module_b/squat/config.py`) — one measured bias, two downstream uses. |
